...
本轮修复针对第一次 LoRA 实验中的短答退化问题，使用了 `answer_style=explain`、assistant-only loss mask、`answer_then_reason` 评测 prompt 和 bad case 分析。当前结果仍然是本地合成 demo 数据上的功能性评测，不等同于正式 benchmark。
...
- `scripts/eval_qwen_vl_lora.py` 的指标包括 `exact_match`、`numeric_match`、`too_short_rate`、`keyword_coverage`、非空回答率、平均回答长度和平均延迟，主要用于快速检查功能闭环。
